#!/bin/bash
#
# VMDIRAC general contextualization script 
...
#
	...
	su dirac -c'python dirac-install -V "VMDIRAC"' >> /var/log/general-context-script.log 2>&1
	...
	unzip vmdirac.zip >> /var/log/general-context-script.log 2>&1
	...
	cd /opt/dirac
        ...
        source bashrc >> /var/log/general-context-script.log 2>&1
	...
	# also the options for the agents: CPUTime, Occi SumbitPools, Site...
		...
		then
			echo "certificates download in dirac-configure at retry: $retry"  >> /var/log/general-context-script.log 2>&1
			break
		fi
		echo "certificates was not download in dirac-configure at retry: $retry"  >> /var/log/general-context-script.log 2>&1
	done
	...
	echo "etc/dirac.cfg content previous to agents run: "  >> /var/log/general-context-script.log 2>&1
	cat etc/dirac.cfg >> /var/log/general-context-script.log 2>&1
	echo >> /var/log/general-context-script.log 2>&1
	# another way to do the same could work when bugfix
	# first generate de proxy server
	#su dirac -c'dirac-proxy-init -C /opt/dirac/etc/grid-security/servercert.pem -K /opt/dirac/etc/grid-security/serverkey.pem' >> /var/log/general-context-script.log 2>&1
	# second configure without UseServerCertificate option
        #su dirac -c'dirac-configure -Hdd -S VMDIRAC-Production -o /LocalSite/CPUTime=1800 -o /LocalSite/SubmitPools=Occi -o /LocalSite/Site=VMDIRAC.develop-pic.es -o /LocalSite/CE=CE-nouse defaults-VMDIRAC.cfg ' >> /var/log/general-context-script.log 2>&1
...
# start the agents: VirtualMachineMonitor, JobAgent
	...
	echo "rights and permissions to control and work JobAgent dirs" >> /var/log/general-context-script.log 2>&1
	mkdir -p /opt/dirac/control/WorkloadManagement/JobAgent >> /var/log/general-context-script.log 2>&1
	mkdir -p /opt/dirac/work/WorkloadManagement/JobAgent >> /var/log/general-context-script.log 2>&1
	chmod 775 /opt/dirac/control/WorkloadManagement/JobAgent >> /var/log/general-context-script.log 2>&1
	chmod 775 /opt/dirac/work/WorkloadManagement/JobAgent >> /var/log/general-context-script.log 2>&1
	chown root:dirac /opt/dirac/work/WorkloadManagement/JobAgent >> /var/log/general-context-script.log 2>&1
	chown root:dirac /opt/dirac/control/WorkloadManagement/JobAgent >> /var/log/general-context-script.log 2>&1
	echo "/opt/dirac/control/WorkloadManagement content" >> /var/log/general-context-script.log 2>&1
	ls -l /opt/dirac/control/WorkloadManagement >> /var/log/general-context-script.log 2>&1
	echo "/opt/dirac/work/WorkloadManagement content" >> /var/log/general-context-script.log 2>&1
	ls -l /opt/dirac/work/WorkloadManagement >> /var/log/general-context-script.log 2>&1
	echo >> /var/log/general-context-script.log 2>&1

	echo "runsvdir startup, have a look to DIRAC JobAgent and VirtualMachineMonitorAgent logs" >> /var/log/general-context-script.log 2>&1
...
#	echo "After DIRAC install:" >> /var/log/general-context-script.log 2>&1
#	ls -l /etc/ssh >> /var/log/general-context-script.log 2>&1
#	chmod 600 /etc/ssh/* >> /var/log/general-context-script.log 2>&1
#	chmod go+r /etc/ssh/ssh_config /etc/ssh/ssh_host_dsa_key.pub /etc/ssh/ssh_host_key.pub /etc/ssh/ssh_host_rsa_key.pub >> /var/log/general-context-script.log 2>&1
#	echo "After restoring rights:" >> /var/log/general-context-script.log 2>&1
#	ls -l /etc/ssh >> /var/log/general-context-script.log 2>&1
