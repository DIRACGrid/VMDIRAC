...
class OcciEndpoint( Endpoint ):
  ...
  def __checkConnection( self ):
    ...
    if self.keystoneUrl is not None:
      keystoneClient = KeystoneClient(self.keystoneUrl, self.authArgs)
      if self.tenant:
        result = keystoneClient.getTenantToken([self.tenant])
      else:
  ...
  def assignFloatingIP( self, nodeID ):
    ...
    print "AT >>> data for requests.post\n", data, network
