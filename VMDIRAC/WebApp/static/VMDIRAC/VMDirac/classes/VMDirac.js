	...
	buildUI : function() {
		...
		me.cbPlot = Ext.create("Ext.form.ComboBox", {
			...
			store : new Ext.data.SimpleStore({
				fields : [ 'value', 'key' ],
				data : [ [ 'load', "Average Load" ], [ 'running', "Running VMs" ], [ 'runningbyendpoint', "Run. VMs By EndPoint" ], [ 'runningbyrunningpod', "Run. VMs By RunningPod" ], [ 'runningbyimage', "Run. VMs By Image" ], [ 'jobs', "Started Jobs" ], [ 'transferbytes', "Transferred Data" ], [ 'transferfiles', "Transferred Files" ] ]
			}),
			displayField : "key",
			valueField : "value",
			labelAlign : "left",
		...
		me.cbRotate.on("change", function(combo, newValue, oldValue, eOpts) {
		...
		me.dataStore = new Ext.data.JsonStore({
			...
			fields : [ 'inst_RunningPod', 'inst_Name', 'inst_Endpoint', 'inst_ErrorMessage', 'inst_InstanceID', 'inst_Status', 'inst_UniqueID', 'img_VMImageID', 'img_Name', 'inst_VMImageID', 'inst_PublicIP', 'inst_LastUpdate', 'inst_Load', 'inst_Uptime', 'inst_Jobs' ],
			remoteSort : true,
			autoLoad : true,
			pageSize : 100,
		...
		me.pagingToolbar.vmStatusCombo.on("change", function(combo, newValue, oldValue, eOpts) {
			var me = this;
			me.oprLoadGridData();
		}, me);
		...
		if (me._can_stop_vm == "Auth") {
			...
			pagingToolbarItems = [ me.btnStopVms, '->', 'VM Status: ', me.pagingToolbar.vmStatusCombo, '-', me.pagingToolbar.updateStamp, '-', 'Items per page: ', me.pagingToolbar.pageSizeCombo, '-' ];

		} else {

			me.btnStopVms = null;
			pagingToolbarItems = [ 'VM Status: ', me.pagingToolbar.vmStatusCombo, '-', me.pagingToolbar.updateStamp, '-', 'Items per page: ', me.pagingToolbar.pageSizeCombo, '-' ];
